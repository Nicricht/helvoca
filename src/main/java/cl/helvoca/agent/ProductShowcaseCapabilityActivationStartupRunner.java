...
@Component
public class ProductShowcaseCapabilityActivationStartupRunner implements ApplicationRunner {
    private static final Logger log =
            LoggerFactory.getLogger(ProductShowcaseCapabilityActivationStartupRunner.class);
    ...
    public ProductShowcaseCapabilityActivationStartupRunner(
            @Value("${HELVOCA_PRODUCT_SHOWCASE_ACTIVATE_ON_STARTUP:false}") boolean enabled,
            @Value("${HELVOCA_PRODUCT_SHOWCASE_BUSINESS_ID:}") String businessId,
    ...
    public void run(ApplicationArguments args) {
        ...
        try {
            ...
            throw new IllegalStateException("HELVOCA_PRODUCT_SHOWCASE_BUSINESS_ID must be a valid UUID", e);
        }
        ...
        databaseContext.runAsTenant(tenantId, () -> {
            ...
            if (result == null) {
                throw new IllegalStateException("Product showcase capability activation returned no result");
            }
            log.info(
                    "PRODUCT_SHOWCASE_CAPABILITY_ACTIVATED businessId={} changed={} createRequest={} listCatalog={} sendWhatsAppOperation={}",
                    tenantId,
                    result.changed(),
                    result.createRequest(),
                    result.listCatalog(),
                    result.sendWhatsAppOperation());
        });
    }
    ...
    ActivationResult activate(UUID businessId) {
        ...
        changed |= capabilities.add(AiCapability.CREATE_REQUEST);
