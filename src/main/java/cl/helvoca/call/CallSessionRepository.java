...
public interface CallSessionRepository extends JpaRepository<CallSession, UUID> {
    ...
    List<CallSession> findTop10ByBusinessIdOrderByStartedAtDesc(UUID businessId);
    long countByBusinessIdAndStartedAtBetween(UUID businessId, Instant from, Instant to);
    long countByBusinessIdAndStatusAndStartedAtBetween(UUID businessId, CallStatus status, Instant from, Instant to);
}
