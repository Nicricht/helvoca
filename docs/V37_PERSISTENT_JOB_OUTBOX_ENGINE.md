...
For outbound messaging, the existing V34 message idempotency key is passed through to the provider. V36 uses the same principle for payment provider creation with `<operationId>` (UUID estable de la operación PAYMENT).
